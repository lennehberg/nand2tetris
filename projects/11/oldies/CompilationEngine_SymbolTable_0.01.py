...
class CompilationEngine:
    ...
    def __init__(self, input_stream: JackTokenizer, symbol_table: SymbolTable, output_stream) -> None:
        """
    ...
    def __get_xml_vm(self, filepath: str) -> tuple:
    ...
    def __write_identifier(self, type="", kind="") -> None:
        """writes the indentifier, adds to symbol table"""
        if self.tokenizer.token_type() == "IDENTIFIER":
            self.sym_table.define(self.tokenizer.identifier(), type, kind)
            if not (type == "" or kind == ""):
                self.out_stream.write(f"<{type}.{kind}.{self.sym_table.index_of(self.tokenizer.identifier())}.identifier> "
    ...
    def __write_var_name(self, type="", kind="") -> None:
